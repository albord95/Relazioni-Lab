...
\begin{center}
	...
	\begin{minipage}[c]{.40\textwidth}
		In un interferometro di Michelson come quello in figura la condizione per avere interferenza costruttiva è \[2(L_1 -L_2)n = m \lambda\] dove $n$ è l'indice di rifrazione dell'aria e $m$ è il numero di frange. Nel nostro apparato variamo $L_2$ e contiamo la corrispondente variazione di $m$.
	\end{minipage}
\end{center}
...
Per farlo inseriamo nel percorso del fascio laser, fra il beam splitter e lo specchio M1, la camera a vuoto mostrata in Figura \ref{fig:cv} e attraverso una pompa facciamo il vuoto al suo interno.\footnote{Per assicurarci di ottenere la migliore condizione di vuoto abbiamo lasciato accesa la pompa finché vedevamo le frange scorrere sul rilevatore, per un tempo medio di circa 5 minuti.}
...
Dopodiché apriamo la camera così che si ristabilisca lentamente la pressione atmosferica al suo interno e contiamo il numero di frange dovute alla variazione dell'indice di rifrazione: il conteggio è stato effettuato osservando il numero di picchi presenti sul grafico mostratoci dal VI sullo schermo del PC. Abbiamo ripetuto le misure due volte per ognuno dei tre laser a nostra disposizione ottenendo entrambe le volte lo stesso numero di picchi; abbiamo quindi riportato tale valore una volta soltanto in Tabella \ref{tab:n}.

\subsection{Analisi dati}
Noto il numero di frange e nota la lunghezza d'onda del laser utilizzato è possibile ricavare la differenza $\Delta$n fra l'indice di rifrazione del vuoto (n=1) e quello dell'aria a pressione atmosferica e a temperatura ambiente tramite la relazione
\begin{equation}
\Delta n = \frac{m\lambda}{2d}
\end{equation}
dove d è la lunghezza della camera a vuoto.

Dal valore di $\Delta$n è quindi facile ricavare l'indice di rifrazione dell'aria presente nella stanza assumendo di essere riusciti a raggiungere all'interno della camera un vuoto tale da avere come condizione iniziale n=1. Abbiamo ottenuto i seguenti valori:
...
Per poter effettuare un confronto con un modello teorico è necessario tenere presente la dipendenza dell'indice di rifrazione, oltre che dalla lunghezza d'onda della luce, dai parametri ambientali quali pressione, temperatura e umidità. Il modello da noi considerato è la versione aggiornata da Birch e Downs dell'equazione di Edlén.\footnote{ K.P. Birch and M.J. Downs, "An updated Edlén equation for the refractive index of air," Metrologia 30, 155-162 (1993)}
Il calcolo dei valori teorici è stato effettuato attraverso un programma presente in rete al sito \texttt{http://emtoolbox.nist.gov/Wavelength/Documentation.asp}

Dato che non siamo a conoscenza dei valori di temperatura, pressione e umidità all'interno del laboratorio abbiamo calcolato dei valori massimi e minimi dell'indice di rifrazione considerando degli intervalli dei parametri ambientali ragionevoli quali: $\Delta$T = 20-25 °C e $\Delta$P = 100-102 kPa.\footnote{L'umidità relativa è stata considerata del 50\% e i valori della pressione sono stati scelti in base al grafico della pressione a Pisa nel mese di Novembre 2017 riportato nel sito \texttt{https://www.woitalia.it/weather/maps/city}}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|c|c|}
		\hline
		laser & $\lambda$ & T=20°C, P=100 kPa & T=25°C, P=102 kPa & $n(\Delta n)$ \\
		\hline
		HeNe & 633 & 1.000268 & 1.000268 &1.000272(6)\\
		rosso & 650 & 1.000268 & 1.000268 &1.000267(7)\\ 
		verde & 532 & 1.000269 & 1.000270 &1.000271(5)\\
		\hline
	\end{tabular}
	\caption{Indice di rifrazione dell'aria}
	\label{tab:n}
...
La precisione delle nostre misure è minore rispetto alla variazione dell'indice di rifrazione dell'aria dovuta ai parametri ambientali, otteniamo quindi dei valori compatibili con quelli teorici entro gli errori.
